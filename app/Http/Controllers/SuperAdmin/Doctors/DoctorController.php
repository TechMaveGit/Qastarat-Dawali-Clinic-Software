...
class DoctorController extends Controller
{
    ...
    {
        $qurDoctor = Doctor::with(['doctorBranch.userBranchName'])->where('role_id', '1');
        // dd($request->all());
        if(isset($request->drname) && $request->drname != ''){
            $qurDoctor->where('name','LIKE',"%{$request->drname}%");
        }
        if(isset($request->status) && $request->status != 0){
            $qurDoctor->where('status',$request->status);
        }
        if(isset($request->branch)){
            $qurDoctor->whereHas('doctorBranch',function ($query) use($request) {
                $query->whereIn('add_branch',$request->branch);
            });
        }

        $data['doctor'] = $qurDoctor->orderBy('id', 'desc')->get();
